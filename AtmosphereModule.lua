...
local ScaleFactor = AtmoSphere:GetScale()
local meta = AtmoSphere:GetMeta()
AtmoSphere:GlobalScope()

function AtmoSphere:GetOffset()
	return (Camera.CFrame.Y + self.AltitudeOffset) * ScaleFactor
end

local x = AtmoSphere:GetOffset()
local SunTextureGui
...
function AtmoSphere:UpdateSun(deltaTime: number)
	...
	if isVisible then -- Converts the Sun's location in the sky to position coordinates of the shine gui.
		...
		meta.SunTexture:TweenSize(
			UDim2.new(
				0, 100 + HorizonElevationSunsetDifference10Ratio * 900*self.SunBrightness*(-((Camera.FieldOfView-70)/200)+1),
				0, 100 + HorizonElevationSunsetDifference10Ratio * 900*self.SunBrightness*(-((Camera.FieldOfView-70)/200)+1)
			),
		...
		meta.SunTexture2:TweenSize(
			UDim2.new(
				0, ((2.5*Camera.ViewportSize.Y*self.SunApparentDiameterRatio)/Camera.FieldOfView)*self.SunBrightness,
